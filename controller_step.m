...
% Update leg targets
stride_eq = (0.92 / phase_rate_eq^0.3 + 0.1*max(abs(X.body.dx) - 1, 0) + 0.2*max(abs(X.body.dx) - 1.8, 0)) * X.body.dx;
if phase.right < cparams.step_lock_phase
    foot_extension = (1 - phase.right) * stride_eq + ...
        0.15 * clamp(X.body.dx - cparams.target_dx, -0.5, 0.5) + ...
        (0.02 + 0.01 * max(abs(X.body.dx) - 1, 0)) * cstate.body_ddx;
    cstate.foot_x_target.right = X.body.x + foot_extension;
end
if phase.left < cparams.step_lock_phase
    foot_extension = (1 - phase.left) * stride_eq + ...
        0.15 * clamp(X.body.dx - cparams.target_dx, -0.5, 0.5) + ...
        (0.02 + 0.01 * max(abs(X.body.dx) - 1, 0)) * cstate.body_ddx;
