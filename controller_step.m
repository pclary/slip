...
length_pd = struct(...
    ...
    'target', {1.0, 1.0, 0.5, 0.0, 0.5, 1.0, 1.0}, ...
    'kp',     {4e3, 4e3, 4e3, 4e3, 4e3, 4e3, 4e3}, ...
    'kd',     {1e2, 1e2, 1e2, 1e2, 1e2, 1e2, 1e2});
weight_ff = struct(...
    'phase', {0.0, 0.3, 0.4, 0.6, 0.7, 1.0}, ...
...
% Update leg targets
foot_extension = X.body.dx / (2 * p.phase_rate * 0.92) + ...
...
u.right.l_eq = ...
    eval_pd(length_pd, s.phase.right, X.right.l_eq, X.right.dl_eq, l_min, l_max) + ...
    eval_ff(weight_ff, s.phase.right) * robot_weight + ...
    eval_ff(energy_ff, s.phase.right) * p.energy_injection;
u.left.l_eq = ...
    eval_pd(length_pd, s.phase.left, X.left.l_eq, X.left.dl_eq, l_min, l_max) + ...
    eval_ff(weight_ff, s.phase.left) * robot_weight + ...
    eval_ff(energy_ff, s.phase.left) * p.energy_injection;

% Angle targets are modulated by footstep target location
theta_eq_target_right = real(asin(complex((s.footstep_target_right - X.body.x) / X.right.l))) - body_theta_st;
u.right.theta_eq = eval_pd(angle_pd, s.phase.right, ...
    X.right.theta_eq, X.right.dtheta_eq, ...
    s.X_laststep.right.theta_eq, theta_eq_target_right);

theta_eq_target_left = real(asin(complex((s.footstep_target_left - X.body.x) / X.left.l))) - body_theta_st;
u.left.theta_eq = eval_pd(angle_pd, s.phase.left, ...
...
u.right.theta_eq = u.right.theta_eq + ...
    eval_pd(body_angle_pd, s.phase.right, body_theta_st, X.body.dtheta, 0, 0);
u.left.theta_eq = u.left.theta_eq + ...
    eval_pd(body_angle_pd, s.phase.left, body_theta_st, X.body.dtheta, 0, 0);
