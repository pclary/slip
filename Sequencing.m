classdef Sequencing < matlab.System
    ...
    methods (Access = protected)
        ...
        function signals = stepImpl(obj, legs)
            ...
            forces = obj.params*(legs([1 7]) - legs([3 9]));
            ...
            obj.foot_contact_good(forces > weight) = true;
            obj.foot_contact_good(forces < weight/2) = false;
            
            feet_fade_rising = threshold_fade(obj.forces_filtered, weight/3, weight/8);
            feet_fade_falling = threshold_fade(obj.forces_filtered, weight/5, 0);
            feet_fade = feet_fade_rising;
            feet_fade(obj.feet_down) = feet_fade_falling(obj.feet_down);
            
            % Detect events
            touchdown = ~obj.feet_down & forces > weight/8;
            midstance = obj.feet_down & obj.forces_filtered < obj.forces_filtered_max - weight/16;
            takeoff = obj.feet_down & forces <= 0;
            
            % Apply events
            obj.feet_down(touchdown) = true;
            obj.feet_down(takeoff) = false;
            obj.forces_filtered_max(touchdown) = 0;
